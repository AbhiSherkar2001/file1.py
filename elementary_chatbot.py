...
theory
This is a rule-based chatbot that simulates a customer service assistant for an IFB Washing Machine Service Centre. It uses regular expressions (regex) to match specific keywords or phrases in the user's message and responds accordingly.

🔁 How It Works:
User Input Handling:
The input is converted to lowercase and trimmed to remove extra spaces.
Predefined Rules:
A dictionary (responses) stores patterns (like "hello", "book service", etc.) as keys and replies as values.

These patterns are written using regex to match multiple variants of similar questions.

Pattern Matching:
The program checks the user’s message against each regex pattern using re.search().
When a match is found, the corresponding response is returned.

Fallback Response:
If no patterns match, a default message is shown asking the user to rephrase the query.

Chat Loop:
A while loop keeps the conversation running until the user types "bye" or "exit".

🧠 Techniques Used:
Regex (Regular Expressions): For flexible matching of user inputs.
Dictionary-Based Logic: Simple and efficient for fixed types of queries.
Looping & Conditionals: For continuous interaction and control flow.
